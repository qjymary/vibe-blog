...
- **[2026-01-18]** 📚 **智能书籍聚合系统 - 博客自动组织成技术书籍**
  - **一键扫描聚合**：自动扫描所有博客，按主题智能分类，聚合成完整技术书籍
  - **智能大纲生成**：LLM 分析博客内容，自动生成章节结构和完整教程大纲
  - **书籍首页生成**：自动生成项目简介、项目亮点、目标受众、前置要求等首页内容
  - **Docsify 书籍阅读器**：类 GitBook 的在线阅读体验，支持侧边栏导航
  - **博客归属标签**：历史记录显示博客所属书籍，一键跳转阅读
  - **强制刷新优化**：重新扫描时同步更新大纲和首页内容
  - **侧边栏优化**：修复前后端共同生成导致的重复问题，明确分工（后端生成章节列表，前端自动提取导读部分）

- **[2026-01-17]** 🎉 **前端下拉框改造 + Prompt 模板化 + 视频动画优化**
  - 新增历史记录分页展示，每页 12 条，支持页码导航
  - 文章类型和文章长度改为下拉框选择，高级选项单行布局
  - 支持任意博客字数模式, 允许配置自定义配置面板支持章节数、配图数、代码块、目标字数设置
  - 视频动画 Prompt 详细优化：7 种元素个性化动画 + 连线虚实流动 + 60% 覆盖率目标
  - 增强 Mermaid 图表渲染：特殊字符处理、双引号转义，提升稳定性


- **[2026-01-17]** 🎨 **多风格配图系统上线**
  - 支持 8 种配图风格：卡通手绘、水墨古风、科研学术、Chiikawa萌系、Biesty剖面图、白板笔记、简约极简、深色科技
  - 封面动画生成功能


- **[2026-01-10]** 📝 **vibe-reviewer 教程评估模块**
  - Git 仓库教程质量评估：深度检查 + 质量审核 + 可读性分析
  - 支持搜索增强评估、SSE 实时进度、Markdown 报告导出
  - 三栏对比视图：文件列表 + Markdown 渲染 + 问题批注

- **[2026-01-05]** 📚 **自定义知识源 2 期**
  - PDF/MD/TXT 文件解析 + 知识分块
  - 图片摘要提取（基于 Qwen-VL）
  - 知识融合写作增强

- **[2025-12-30]** 🚀 **vibe-blog v0.1.0 首次发布**
